...
<link href="<?php echo $domain; ?>/assets/css/v10.style.p.min.css" rel="stylesheet" media="all">
</head>
<body>
...
<div class="container-fluid containerfiltersmobile">
    <button class="showfilters" onclick="openMyFilters()" >filtres</button>
    <button class="changecars" onclick="openMyRide()">modifier véhicule</button>
</div>
    ...
    <div class="container-fluid mymaxwidth">
		...
		<div class="row">
			<div id="myfilters" class="col-md-4 col-lg-3 containerfilters text-left 100vh">
					...
					<?php
				    $query_get_pg_filter = "SELECT DISTINCT PIECE_FIL_ID, PIECE_FIL_NAME, 
					...
					<form>
						...
						{
						$this_pg_name = $result_get_pg_filter['PIECE_FIL_NAME'];
						$this_pg_alias = url_title($result_get_pg_filter['PIECE_FIL_NAME']);
				    	?>
						<label>
						<input type="checkbox" name="fl-pgselect" 
						value="<?php echo $this_pg_alias; ?>" 
						id="<?php echo $this_pg_alias; ?>" />
						&nbsp; &nbsp;<?php echo $result_get_pg_filter['PIECE_FIL_NAME']." (".$result_get_pg_filter['NBP'].")"; ?></label>
						<?php
						}
						?>
					</form>
				    ...
				    <form>
						...
						{
						$this_psf_name = $result_get_technical_filter_essieu['PSF_SIDE'];
						$this_psf_alias = url_title($result_get_technical_filter_essieu['PSF_SIDE']);
				    	?>
						<label>
						<input type="checkbox" name="fl-psfselect" 
						value="<?php echo $this_psf_alias; ?>" 
						id="<?php echo $this_psf_alias; ?>" />
					...
					//{
					// CROSS FAMILY SEULEMENT POUR LA PAGE PC
					$mc_mf_prime=$mf_id;	
					...
					{
					?>
				    <div class="container-fluid samefamilyselect">

				    	<div></div>
						<b>Catalogue <?php echo $mf_name_site; ?></b>
						<?php
						while($result_get_cross_gamme = $request_get_cross_gamme->fetch_assoc())
						{
						$this_pg_name = $result_get_cross_gamme['PG_NAME'];
						$thislinktoPage = $domain."/".$Piece."/".$result_get_cross_gamme['PG_ALIAS']."-".$result_get_cross_gamme['PG_ID']."/".$marque_alias."-".$marque_id."/".$modele_alias."-".$modele_id."/".$type_alias."-".$type_id.".html";
						?>
						<br>
			...
			<div class="col-md-8 col-lg-9 text-left">
				...
				<div class="row">
				    <div class="col-12 headerlist text-left d-none d-md-block">
				        <b><?php echo $GammeCarCountArticle; ?></b> produits disponibles
				        <button class="changecars" onclick="openMyRide()">modifier véhicule</button>
				    </div>
				    <div id="myride" class="col-12 containerride">

						<div class="container-fluid containerSeekCar">
						<div class="row">
						<div class="col-12 pb-3">
						Sélectionnez votre véhicule <i class="pe-7s-close" onclick="closeMyRide()"></i>
						</div>
						<div class="col-12 col-sm-6 col-md-3 p-1 pb-0">
				    ...
				    <div class="col-12 p-0">
						...
						<div class="flowers">
						<?php
				        $query_bloc_listing = "SELECT DISTINCT PIECE_FIL_ID, PSF_ID , PIECE_FIL_NAME , PSF_SIDE
						FROM PIECES_RELATION_TYPE
						JOIN PIECES_SIDE_FILTRE ON PSF_ID = RTP_PSF_ID
						JOIN PIECES ON PIECE_ID = RTP_PIECE_ID
						WHERE RTP_TYPE_ID = $type_id AND RTP_PG_ID = $pg_id AND PIECE_DISPLAY = 1";
						$request_bloc_listing = $conn->query($query_bloc_listing);
						if ($request_bloc_listing->num_rows > 0) 
						{
				        while ($result_bloc_listing = $request_bloc_listing->fetch_assoc()) 
				        {
				        $filter_side_id = $result_bloc_listing['PSF_ID'];
						$filter_id = $result_bloc_listing['PIECE_FIL_ID'];
						$filter_pg_alias = url_title($result_bloc_listing['PIECE_FIL_NAME']);
						$filter_psf_alias = url_title($result_bloc_listing['PSF_SIDE']);
						$filter_flower_id = $filter_pg_alias.$filter_psf_alias;
				        ?>
				        <div class="flower itemsTitle" data-id="<?php echo $filter_flower_id; ?>" data-category="<?php echo $filter_pg_alias; ?> <?php echo $filter_psf_alias; ?>">

				        	<h2><?php echo $result_bloc_listing['PIECE_FIL_NAME']." ".$result_bloc_listing['PSF_SIDE']; ?></h2>
				        	<div></div>

				        </div>
				        <?php
				        $query_listing = "SELECT DISTINCT PIECE_ID, PIECE_REF, PIECE_REF_CLEAN, PIECE_DES, PIECE_NAME, PIECE_HAS_IMG, PIECE_HAS_OEM, PIECE_QTY_SALE, PIECE_QTY_PACK,  
							COALESCE(PSF_SIDE,PIECE_NAME_SIDE) AS PIECE_NAME_SIDE, PIECE_NAME_COMP, 
							PM_ID, PM_NAME, PM_ALIAS, PM_LOGO, COALESCE(PIECE_DES,PM_QUALITY) AS PM_QUALITY, PM_OES, PM_NB_STARS, 
							PIECE_FIL_NAME , PSF_SIDE 
							FROM PIECES_RELATION_TYPE
							JOIN PIECES_SIDE_FILTRE ON PSF_ID = RTP_PSF_ID
							JOIN PIECES ON PIECE_ID = RTP_PIECE_ID
							...
							JOIN PIECES_PRICE ON PRI_PIECE_ID = PIECE_ID
							WHERE RTP_TYPE_ID = $type_id AND RTP_PG_ID = $pg_id
							AND RTP_PSF_ID = $filter_side_id AND PIECE_FIL_ID = $filter_id
							AND PIECE_DISPLAY = 1 AND PM_DISPLAY = 1";
						$request_listing = $conn->query($query_listing);
				        while ($result_listing = $request_listing->fetch_assoc()) 
				        {
				        $this_piece_name= $result_listing['PIECE_NAME']." ".$result_listing['PIECE_NAME_SIDE']." ".$result_listing['PIECE_NAME_COMP'];
				        $piece_id_this = $result_listing['PIECE_ID'];
				        $this_piece_ref = $result_listing['PIECE_REF'];
				        $this_pm_id = $result_listing['PM_ID'];
				        $this_pm_name = $result_listing['PM_NAME'];
				        $this_pm_quality = $result_listing['PM_OES'];
				        if($this_pm_quality == 'A') $this_pm_quality = "AFTERMARKET";
				        else $this_pm_quality = "OES";
				        $this_pm_stars = $result_listing['PM_NB_STARS'];
						$piece_has_oem_this = $result_listing['PIECE_HAS_OEM'];
				        $filter_pm_alias = $result_listing['PM_ALIAS'];
				        $filter_pm_quality_alias = url_title($this_pm_quality);
						$filter_pg_alias = url_title($result_listing['PIECE_FIL_NAME']);
						$filter_psf_alias = url_title($result_listing['PSF_SIDE']);
						$filter_pm_stars = $result_listing['PM_NB_STARS'];
						...
						{
							$result_listing_price = $request_item_list_price->fetch_assoc();
							$price_PV_TTC = $result_listing_price['PRI_VENTE_TTC'] * $result_listing['PIECE_QTY_SALE'];
							$price_CS_TTC = $result_listing_price['PRI_CONSIGNE_TTC'] * $result_listing['PIECE_QTY_SALE'];
						}
						...
						// PHOTO
						if($result_listing['PIECE_HAS_IMG']==1)
						{
							$query_item_list_img = "SELECT CONCAT('rack/',PMI_FOLDER,'/',PMI_NAME) AS PIECE_IMG 
							...
							{
								$result_listing_img = $request_item_list_img->fetch_assoc();
								$photo_link = $result_listing_img['PIECE_IMG'];
								$photo_alt = $result_listing['PIECE_NAME']." ".$result_listing['PIECE_NAME_SIDE']." ".$result_listing['PIECE_NAME_COMP']." ".$result_listing['PM_NAME']." ".$result_listing['PIECE_REF'];
								$photo_title = $result_listing['PIECE_NAME']." ".$result_listing['PIECE_NAME_SIDE']." ".$result_listing['PIECE_NAME_COMP']." ".$marque_name_site." ".$modele_name_site. " ".$result_listing['PM_NAME']." ".$result_listing['PIECE_REF'];
							}
							else
							{
				        ...
				        ?>
<div class="flower" data-id="ref<?php echo $piece_id_this; ?>" data-category="<?php echo $filter_pg_alias; ?> <?php echo $filter_psf_alias; ?> <?php echo $filter_pm_quality_alias; ?> <?php echo $filter_pm_stars_alias; ?> <?php echo $filter_pm_alias; ?>">
							...
							<div class="itemsBloc">
								...
								if($isMacVersion == false)
								{
									$this_pm_img = $result_listing['PM_LOGO'];
								}
								else
								{
									$this_pm_img = str_replace(".webp",".png",$result_listing['PM_LOGO']);
								}
								?>
								...
								?>
								<b><?php echo $this_piece_name; ?><br><?php echo $this_pm_name; ?> <?php echo $this_piece_ref; ?></b>
								...
								/*$query_item_list_technical = "SELECT DISTINCT PCL_CRI_ID, PCL_CRI_CRITERIA, PC_CRI_VALUE AS PCL_VALUE, 
									...
									ORDER BY PCL_LEVEL, PCL_SORT,PCL_CRI_CRITERIA LIMIT 3";
								$request_item_list_technical = $conn->query($query_item_list_technical);
								if($request_item_list_technical->num_rows)
								{
								?>
									<?php
									while($result_listing_technical= $request_item_list_technical->fetch_assoc())
									{
										echo $result_listing_technical["PCL_CRI_CRITERIA"]." : <span>".$result_listing_technical["PCL_VALUE"]." ".$result_listing_technical["PCL_CRI_UNIT"]."</span><br>";
									}
									?>
								<?php
								} */
								...
								aria-label="Fiche" 
								data-whatever="<?php echo $piece_id_this; ?>/<?php echo $type_id; ?>"><i class="pe-7s-look"></i></button>
    ...
    <div class="container-fluid mymaxwidth">
		...
		<div class="row">
			...
			<div class="col-12">
				...
				<p>
					<?php echo $pagepreview; ?>...<br><?php echo $pagecontent; ?>
				</p>
