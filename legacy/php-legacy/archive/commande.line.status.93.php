...
if(isset($_POST["update"]))
{
	...
	$StatusUpdateQuery = "UPDATE ___XTR_ORDER_LINE SET ORL_ORLS_ID = $orls_id, ORL_EQUIV_ID = $orl_id_go_back  
		WHERE ORL_ORD_ID = $ord_id AND ORL_ID = $orl_id";
	if($conn->query($StatusUpdateQuery) === TRUE)
	{
		$StatusUpdateQuery = "UPDATE ___XTR_ORDER_LINE SET ORL_EQUIV_ID = 0  
			WHERE ORL_ORD_ID = $ord_id AND ORL_ID = $orl_id_go_back";
		if($conn->query($StatusUpdateQuery) === TRUE)
		{
			$StatusUpdateResponse = "Proposition d'équivalence refusée avec Succès.";
...
<body class="BODY-MODAL">
	...
	<div class="row">
		<div class="col-lg-12 text-center pb-3">
			Etes vous sûr de vouloir procéder au <b><?php echo $pageAction; ?></b> de cette pièce :<br>
			...
			<br>
			Cette opération est définitive et irréversible et entrainera l'annulation de proposition déjà envoyé au client, Merci de confirmer.
		</div>
		<div class="col-lg-12 text-center pt-3">
			<input type="submit" value="Valider le <?php echo $pageAction; ?>" class="TAKE-ACTION-SUBMIT-DELETE-LINK" style="float: none;" />
