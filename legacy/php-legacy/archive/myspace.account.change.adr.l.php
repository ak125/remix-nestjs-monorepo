...
// UPDATE DE COMPTE
	...
	$ask2updateQuery = "UPDATE backofficeplateform_client SET livciv = '$civ', livnom = '$nom', livpre = '$prenom',  
		livtel = '$tel', livport = '$gsm', livadr = '$adr', livzipcode = '$zipcode', 
		livville = '$ville', livpays = '$pays'  
	...
	else
	{
		...
		$ask2updateResponseMsg .= "Une erreur a surgie lors de la modification de votre adresse de livraison :<br>".$conn->error;
	}
...
// GET CLIENT DATA
    ...
    $result_client_data = $request_client_data->fetch_assoc();
		...
		// Facture data
		$connectedclt_liv_mail = $result_client_data['livmail'];
		$connectedclt_liv_civ = $result_client_data['livciv'];
		$connectedclt_liv_nom = $result_client_data['livnom'];
		$connectedclt_liv_prenom = $result_client_data['livpre'];
		$connectedclt_liv_adr = $result_client_data['livadr'];
		$connectedclt_liv_tel = $result_client_data['livtel'];
		$connectedclt_liv_port = $result_client_data['livport'];
		$connectedclt_liv_zipcode = $result_client_data['livzipcode'];
		$connectedclt_liv_ville = $result_client_data['livville'];
		$connectedclt_liv_pays = $result_client_data['livpays'];
// FIN CHANG. CONNEXION
?>
...
<form action="" method="post" role="form">
    ...
    <div class="row">
	    <div class="col-12 pt-3 pb-3 text-justify">
			Cette interface vous permettra de mettre à jours l'ensemble des données (nom, prénom, adresse relatives à la livraison des colis du compte <?php echo $mailclt; ?>. 
			<br>
    ...
    <div class="row">
        <div class="col-3 col-md-2 pb-3">
            ...
            <select name="civ" class="subscribe">
                <option <?php if ($connectedclt_liv_civ=="Mr.") { ?> selected="selected" <?php } ?> >Mr.</option>
				<option <?php if ($connectedclt_liv_civ=="Mlle.") { ?> selected="selected" <?php } ?> >Mlle.</option>
				<option <?php if ($connectedclt_liv_civ=="Mme.") { ?> selected="selected" <?php } ?> >Mme.</option>
            </select>
        </div>
        <div class="col-9 col-md-5 pb-3">
            Nom *
            <input type="text" name="nom" class="subscribe" value="<?php echo $connectedclt_liv_nom; ?>" required autocomplete="off"/>
        </div>
        <div class="col-12 col-md-5 pb-3">
            Prénom
            <input type="text" name="prenom" class="subscribe" value="<?php echo $connectedclt_liv_prenom; ?>" autocomplete="off"/>
        </div>
        <div class="col-12 pb-3">
            Email
            <input type="text" name="mail" class="subscribe" value="<?php echo $connectedclt_liv_mail; ?>" disabled="disabled"/>
          
        </div>
        <div class="col-12 col-md-6 pb-3">
            Adresse *
            <input type="text" name="adr" class="subscribe" value="<?php echo $connectedclt_liv_adr; ?>" required autocomplete="off"/>
          
        </div>
        <div class="col-12 col-md-2 pb-3">
            Code postal *
            <input type="number" name="zipcode" class="subscribe" value="<?php echo $connectedclt_liv_zipcode; ?>" required autocomplete="off"/>
        </div>
        <div class="col-12 col-md-2 pb-3">
            Ville *
            <input type="text" name="ville" class="subscribe" value="<?php echo $connectedclt_liv_ville; ?>" required autocomplete="off"/>
        </div>
        <div class="col-12 col-md-2 pb-3">
            Pays *
            <select name="pays" class="subscribe">
              <option <?php if ($connectedclt_liv_pays=="France") { ?> selected="selected" <?php } ?> >France</option>
            </select>
        </div>
        <div class="col-12 col-md-6 pb-3">
            Téléphone
            <input type="number" name="tel" class="subscribe" value="<?php echo $connectedclt_liv_tel; ?>" autocomplete="off"/>
        </div>
        <div class="col-12 col-md-6 pb-3">
            Gsm *
            <input type="number" name="gsm" class="subscribe" value="<?php echo $connectedclt_liv_port; ?>" required autocomplete="off"/>
        </div>
    </div>
