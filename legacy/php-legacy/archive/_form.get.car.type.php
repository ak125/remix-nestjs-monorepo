...
if(($formCarMarqueid>0)&&($formCarMarqueYear>0)&&($formCarModelid>0))
{
        ...
        $formCarMotorlabel = $result_get_car_in_type_fuel["TMF_MOTOR"];
            ...
            $query_get_car_in_type = "SELECT TYPE_ID, TYPE_ALIAS, TYPE_NAME, TYPE_POWER_PS, TYPE_YEAR_FROM, 
              COALESCE(TYPE_YEAR_TO,'$thisYear') AS TYPE_TO, MODELE_ALIAS, MARQUE_ALIAS, TYPE_SORT 
              FROM AUTO_TYPE
              JOIN AUTO_MODELE ON MODELE_ID = TYPE_MODELE_ID
              JOIN AUTO_MARQUE ON MARQUE_ID = TYPE_MARQUE_ID
              WHERE TYPE_TMF_ID = $formCarMotorid AND TYPE_MODELE_ID = $formCarModelid AND TYPE_DISPLAY = 1 
			  AND TYPE_YEAR_FROM <= $formCarMarqueYear AND COALESCE(TYPE_YEAR_TO,'$thisYear') >= $formCarMarqueYear
              ORDER BY TYPE_SORT";
    ...
    $formCarLink = $domain."/".$Auto."/".$result_get_car_in_type['MARQUE_ALIAS']."-".$formCarMarqueid."/".$result_get_car_in_type['MODELE_ALIAS']."-".$formCarModelid."/".$result_get_car_in_type["TYPE_ALIAS"]."-".$result_get_car_in_type["TYPE_ID"].".html";
    ?>
